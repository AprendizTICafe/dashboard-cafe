...
class GestorMarketing(models.Model):
    """Gestor do departamento de Marketing"""
    GestorID = models.AutoField(primary_key=True)  # ID único para cada gestor de Marketing
    ...
    def __str__(self):
        return f"{self.GestorID} - {self.Name} ({self.Email}) - Marketing"
...
class WarningRequest(models.Model):
    """Solicitação de advertência criada pelo Gestor de Marketing"""
    REQUEST_STATUS = [
        ('criada', 'Criada'),
    ...
    GestorMarketing = models.ForeignKey(GestorMarketing, on_delete=models.CASCADE)  # Gestor Marketing responsável
    ColaboradorID = models.ForeignKey(Colaborador, on_delete=models.CASCADE, related_name='marketing_warning_requests')  # Colaborador que será advertido
