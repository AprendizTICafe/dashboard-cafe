...
def advertencia(request):
    ...
    context = {
        ...
        'advertencias_solicitadas': Warnings.objects.filter(DepartmentOrigin="Marketing", Active=True, CurrentStage='criada').count(),
        'advertencias_em_analise_rh': Warnings.objects.filter(DepartmentOrigin="Marketing", Active=True, CurrentStage='analise_rh').count(),
        'advertencias_em_analise_diretoria': Warnings.objects.filter(DepartmentOrigin="Marketing", Active=True, CurrentStage='analise_diretoria').count(),
        'advertencias_aprovadas': Warnings.objects.filter(DepartmentOrigin="Marketing", Active=True, CurrentStage='aprovada').count(),
        'advertencias_concluidas': Warnings.objects.filter(DepartmentOrigin="Marketing", Active=True, CurrentStage='concluida').count(),
        
        # Filta as advertências ativas e filtra as 10 últimas.
        'ultimas_advertencias': Warnings.objects.filter(DepartmentOrigin="Marketing", Active=True).order_by('-CreatedAt')[:10],
    ...
    return render(request, 'Marketing/advertencia.html', context)
...
def nova_advertencia(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect('marketing:advertencia') # Redireciona para atualizar as estatísticas

        except Exception as e:
            # Em caso de erro, exibe a mensagem e volta ao portal
            messages.error(request, f"Erro ao processar solicitação: {e}")
            return redirect('marketing:advertencia')

    # --- Lógica para GET (Exibir formulário) ---
    # Filtramos diretamente pelo departamento de Marketing e colaboradores ativos
    # Isso garante que a lista apareça mesmo se o Gestor não tiver cadastro na tabela Marketing.models.Gestor ainda
    colaboradores = Colaborador.objects.filter(Department="Marketing", Active=True)

    return render(request, 'Marketing/nova_advertencia.html', {'colaboradores': colaboradores.order_by('Name')})

def advertencia_list(request):
    # 1. Filtra por departamento de Marketing (para o app Marketing)
    # Lista de colaboradores para o <select> do filtro
    colaboradores_dept = Colaborador.objects.filter(Department="Marketing", Active=True).order_by('Name')
    queryset = Warnings.objects.filter(DepartmentOrigin="Marketing")
...
def view_advertencia(request, id):
    """
    Exibe os detalhes de uma advertência criada por Marketing.
    ...
    return render(request, 'Marketing/detalhes_advertencia.html', context)
...
def send_to_rh(request, id):
    """
    Envia a advertência de Marketing para análise do RH.
    ...
    try:
        with transaction.atomic():
            ...
            advertencia.CurrentStage = 'Em Análise RH'
        ...
        return redirect('marketing:view_advertencia', id=id)
