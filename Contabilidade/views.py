...
def advertencia(request):
    ...
    context = {
        ...
        'advertencias_solicitadas': Warnings.objects.filter(DepartmentOrigin="Contabilidade", Active=True, CurrentStage='criada').count(),
        'advertencias_em_analise_rh': Warnings.objects.filter(DepartmentOrigin="Contabilidade", Active=True, CurrentStage='analise_rh').count(),
        'advertencias_em_analise_diretoria': Warnings.objects.filter(DepartmentOrigin="Contabilidade", Active=True, CurrentStage='analise_diretoria').count(),
        'advertencias_aprovadas': Warnings.objects.filter(DepartmentOrigin="Contabilidade", Active=True, CurrentStage='aprovada').count(),
        'advertencias_concluidas': Warnings.objects.filter(DepartmentOrigin="Contabilidade", Active=True, CurrentStage='concluida').count(),
        
        # Filta as advertências ativas e filtra as 10 últimas.
        'ultimas_advertencias': Warnings.objects.filter(DepartmentOrigin="Contabilidade", Active=True).order_by('-CreatedAt')[:10],
    ...
    return render(request, 'Contabilidade/advertencia.html', context)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                nova_war = Warnings.objects.create(
                    ...
                    DepartmentOrigin="Contabilidade",
                    Gestor=usuario,  
            ...
            return redirect('contabilidade:advertencia') # Redireciona para atualizar as estatísticas

        except Exception as e:
            # Em caso de erro, exibe a mensagem e volta ao portal
            messages.error(request, f"Erro ao processar solicitação: {e}")
            return redirect('contabilidade:advertencia')

    # --- Lógica para GET (Exibir formulário) ---
    # Filtramos diretamente pelo departamento de Contabilidade e colaboradores ativos
    # Isso garante que a lista apareça mesmo se o Gestor não tiver cadastro na tabela Contabilidade.models.Gestor ainda
    colaboradores = Colaborador.objects.filter(Department="Contabilidade", Active=True)

    return render(request, 'Contabilidade/nova_advertencia.html', {'colaboradores': colaboradores.order_by('Name')})

def advertencia_list(request):
    # 1. Filtra por departamento de Contabilidade (para o app Contabilidade)
    # Lista de colaboradores para o <select> do filtro
    colaboradores_dept = Colaborador.objects.filter(Department="Contabilidade", Active=True).order_by('Name')
    queryset = Warnings.objects.filter(DepartmentOrigin="Contabilidade")
...
def view_advertencia(request, id):
    """
    Exibe os detalhes de uma advertência criada por Contabilidade.
    ...
    return render(request, 'Contabilidade/detalhes_advertencia.html', context)
...
def send_to_rh(request, id):
    """
    Envia a advertência de Contabilidade para análise do RH.
    ...
    try:
        ...
        return redirect('contabilidade:view_advertencia', id=id)
