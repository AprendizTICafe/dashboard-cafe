...
class GestorContabilidade(models.Model):
    """Gestor do departamento de Contabilidade"""
    GestorID = models.AutoField(primary_key=True)  # ID único para cada gestor de Contabilidade
    ...
    def __str__(self):
        return f"{self.GestorID} - {self.Name} ({self.Email}) - Contabilidade"

    class Meta:
        verbose_name = "Gestor Contabilidade"
        verbose_name_plural = "Gestores Contabilidade"
        db_table = "contabilidade_gestor_contabilidade"


class WarningRequest(models.Model):
    """Solicitação de advertência criada pelo Gestor de Contabilidade"""
    REQUEST_STATUS = [
        ('criada', 'Criada'),
    ...
    GestorContabilidade = models.ForeignKey(GestorContabilidade, on_delete=models.CASCADE)  # Gestor Contabilidade responsável
    ColaboradorID = models.ForeignKey(Colaborador, on_delete=models.CASCADE, related_name='contabilidade_warning_requests')  # Colaborador que será advertido
    ...
    class Meta:
        ...
        db_table = "contabilidade_warning_request"
