...
class GestorTI(models.Model):
    """Gestor do departamento de TI"""
    GestorID = models.AutoField(primary_key=True)  # ID único para cada gestor de TI
    ...
    def __str__(self):
        return f"{self.GestorID} - {self.Name} ({self.Email}) - TI"
...
class WarningRequest(models.Model):
    """Solicitação de advertência criada pelo Gestor de TI"""
    REQUEST_STATUS = [
        ('criada', 'Criada'),
    ...
    GestorTI = models.ForeignKey(GestorTI, on_delete=models.CASCADE)  # Gestor TI responsável
    ColaboradorID = models.ForeignKey(Colaborador, on_delete=models.CASCADE, related_name='ti_warning_requests')  # Colaborador que será advertido
