...
class GestorLogistica(models.Model):
    """Gestor do departamento de Logística"""
    GestorID = models.AutoField(primary_key=True)  # ID único para cada gestor de Logística
    ...
    def __str__(self):
        return f"{self.GestorID} - {self.Name} ({self.Email}) - Logística"

    class Meta:
        verbose_name = "Gestor Logística"
        verbose_name_plural = "Gestores Logística"
        db_table = "logistica_gestor_logistica"


class WarningRequest(models.Model):
    """Solicitação de advertência criada pelo Gestor de Logística"""
    REQUEST_STATUS = [
        ('criada', 'Criada'),
    ...
    GestorLogistica = models.ForeignKey(GestorLogistica, on_delete=models.CASCADE)  # Gestor Logística responsável
    ColaboradorID = models.ForeignKey(Colaborador, on_delete=models.CASCADE, related_name='logistica_warning_requests')  # Colaborador que será advertido
    ...
    class Meta:
        ...
        db_table = "logistica_warning_request"
