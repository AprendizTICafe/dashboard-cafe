...
class GestorFinanceiro(models.Model):
    """Gestor do departamento de Financeiro"""
    GestorID = models.AutoField(primary_key=True)  # ID único para cada gestor de Financeiro
    ...
    def __str__(self):
        return f"{self.GestorID} - {self.Name} ({self.Email}) - Financeiro"
...
class WarningRequest(models.Model):
    """Solicitação de advertência criada pelo Gestor de Financeiro"""
    REQUEST_STATUS = [
        ('criada', 'Criada'),
    ...
    GestorFinanceiro = models.ForeignKey(GestorFinanceiro, on_delete=models.CASCADE)  # Gestor Financeiro responsável
    ColaboradorID = models.ForeignKey(Colaborador, on_delete=models.CASCADE, related_name='financeiro_warning_requests')  # Colaborador que será advertido
