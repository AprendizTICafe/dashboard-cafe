...
def advertencia(request):
    ...
    stats = Warnings.objects.filter(
        DepartmentOrigin=DEPARTMENTS['FINANCEIRO'],
        Active=True
    ).aggregate(
    ...
    return render(request, 'Financeiro/advertencia.html', context)
...
def nova_advertencia(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not is_valid:
                ...
                return redirect('financeiro:advertencia')
    ...
    return render(request, 'Financeiro/nova_advertencia.html', {'colaboradores': colaboradores.order_by('Name')})

def advertencia_list(request):
    # 1. Filtra por departamento de Financeiro (para o app Financeiro)
    # Lista de colaboradores para o <select> do filtro
    colaboradores_dept = Colaborador.objects.filter(Department="Financeiro", Active=True).order_by('Name')
    queryset = Warnings.objects.filter(DepartmentOrigin="Financeiro")
...
def view_advertencia(request, id):
    """
    Exibe os detalhes de uma advertência criada por Financeiro.
    ...
    return render(request, 'Financeiro/detalhes_advertencia.html', context)
...
def send_to_rh(request, id):
    """
    Envia a advertência de Financeiro para análise do RH.
    ...
    try:
        ...
        return redirect('financeiro:view_advertencia', id=id)
