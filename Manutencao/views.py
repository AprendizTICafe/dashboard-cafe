...
def advertencia(request):
    ...
    context = {
        ...
        'advertencias_solicitadas': Warnings.objects.filter(DepartmentOrigin="Manutenção", Active=True, CurrentStage='criada').count(),
        'advertencias_em_analise_rh': Warnings.objects.filter(DepartmentOrigin="Manutenção", Active=True, CurrentStage='analise_rh').count(),
        'advertencias_em_analise_diretoria': Warnings.objects.filter(DepartmentOrigin="Manutenção", Active=True, CurrentStage='analise_diretoria').count(),
        'advertencias_aprovadas': Warnings.objects.filter(DepartmentOrigin="Manutenção", Active=True, CurrentStage='aprovada').count(),
        'advertencias_concluidas': Warnings.objects.filter(DepartmentOrigin="Manutenção", Active=True, CurrentStage='concluida').count(),
        
        # Filta as advertências ativas e filtra as 10 últimas.
        'ultimas_advertencias': Warnings.objects.filter(DepartmentOrigin="Manutenção", Active=True).order_by('-CreatedAt')[:10],
    ...
    return render(request, 'Manutencao/advertencia.html', context)
...
def nova_advertencia(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect('manutencao:advertencia') # Redireciona para atualizar as estatísticas

        except Exception as e:
            # Em caso de erro, exibe a mensagem e volta ao portal
            messages.error(request, f"Erro ao processar solicitação: {e}")
            return redirect('manutencao:advertencia')

    # --- Lógica para GET (Exibir formulário) ---
    # Filtramos diretamente pelo departamento de Manutenção e colaboradores ativos
    # Isso garante que a lista apareça mesmo se o Gestor não tiver cadastro na tabela Manutencao.models.Gestor ainda
    colaboradores = Colaborador.objects.filter(Department="Manutenção", Active=True)

    return render(request, 'Manutencao/nova_advertencia.html', {'colaboradores': colaboradores.order_by('Name')})

def advertencia_list(request):
    # 1. Filtra por departamento de Manutenção (para o app Manutencao)
    # Lista de colaboradores para o <select> do filtro
    colaboradores_dept = Colaborador.objects.filter(Department="Manutenção", Active=True).order_by('Name')
    queryset = Warnings.objects.filter(DepartmentOrigin="Manutenção")
...
def view_advertencia(request, id):
    """
    Exibe os detalhes de uma advertência criada por Manutenção.
    ...
    return render(request, 'Manutencao/detalhes_advertencia.html', context)
...
def send_to_rh(request, id):
    """
    Envia a advertência de Manutenção para análise do RH.
    ...
    try:
        ...
        return redirect('manutencao:view_advertencia', id=id)
