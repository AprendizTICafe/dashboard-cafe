...
def advertencia(request):
    ...
    context = {
        ...
        'advertencias_solicitadas': Warnings.objects.filter(DepartmentOrigin="Segurança do Trabalho", Active=True, CurrentStage='criada').count(),
        'advertencias_em_analise_rh': Warnings.objects.filter(DepartmentOrigin="Segurança do Trabalho", Active=True, CurrentStage='analise_rh').count(),
        'advertencias_em_analise_diretoria': Warnings.objects.filter(DepartmentOrigin="Segurança do Trabalho", Active=True, CurrentStage='analise_diretoria').count(),
        'advertencias_aprovadas': Warnings.objects.filter(DepartmentOrigin="Segurança do Trabalho", Active=True, CurrentStage='aprovada').count(),
        'advertencias_concluidas': Warnings.objects.filter(DepartmentOrigin="Segurança do Trabalho", Active=True, CurrentStage='concluida').count(),
        
        # Filta as advertências ativas e filtra as 10 últimas.
        'ultimas_advertencias': Warnings.objects.filter(DepartmentOrigin="Segurança do Trabalho", Active=True).order_by('-CreatedAt')[:10],
    ...
    return render(request, 'SegurancaTrabalho/advertencia.html', context)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                nova_war = Warnings.objects.create(
                    ...
                    DepartmentOrigin="Segurança do Trabalho",
                    Gestor=usuario,  
            ...
            return redirect('segurancatrabalho:advertencia') # Redireciona para atualizar as estatísticas

        except Exception as e:
            # Em caso de erro, exibe a mensagem e volta ao portal
            messages.error(request, f"Erro ao processar solicitação: {e}")
            return redirect('segurancatrabalho:advertencia')

    # --- Lógica para GET (Exibir formulário) ---
    # Filtramos diretamente pelo departamento de Segurança do Trabalho e colaboradores ativos
    # Isso garante que a lista apareça mesmo se o Gestor não tiver cadastro na tabela SegurancaTrabalho.models.Gestor ainda
    colaboradores = Colaborador.objects.filter(Department="Segurança do Trabalho", Active=True)

    return render(request, 'SegurancaTrabalho/nova_advertencia.html', {'colaboradores': colaboradores.order_by('Name')})

def advertencia_list(request):
    # 1. Filtra por departamento de Segurança do Trabalho (para o app SegurancaTrabalho)
    # Lista de colaboradores para o <select> do filtro
    colaboradores_dept = Colaborador.objects.filter(Department="Segurança do Trabalho", Active=True).order_by('Name')
    queryset = Warnings.objects.filter(DepartmentOrigin="Segurança do Trabalho")
...
def view_advertencia(request, id):
    """
    Exibe os detalhes de uma advertência criada por Segurança do Trabalho.
    ...
    return render(request, 'SegurancaTrabalho/detalhes_advertencia.html', context)
...
def send_to_rh(request, id):
    """
    Envia a advertência de Segurança do Trabalho para análise do RH.
    ...
    try:
        ...
        return redirect('segurancatrabalho:view_advertencia', id=id)
